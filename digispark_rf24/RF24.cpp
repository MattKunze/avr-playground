...
uint8_t RF24::doTransfer(uint8_t byte)
{
#ifdef HARDWARE_SPI
  return doTransfer(byte);
#else
  TinyWireM.send(byte);
...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = doTransfer( R_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
    *buf++ = doTransfer(0xff);
...
{
  csn(LOW);
  doTransfer( R_REGISTER | ( REGISTER_MASK & reg ) );
  uint8_t result = doTransfer(0xff);
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = doTransfer( W_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
    doTransfer(*buf++);
...
uint8_t RF24::write_payload(const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = doTransfer( W_TX_PAYLOAD );
  while ( data_len-- )
    doTransfer(*current++);
  while ( blank_len-- )
    doTransfer(0);
...
uint8_t RF24::read_payload(void* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = doTransfer( R_RX_PAYLOAD );
  while ( data_len-- )
    *current++ = doTransfer(0xff);
  while ( blank_len-- )
    doTransfer(0xff);
  csn(HIGH);
...
uint8_t RF24::getDynamicPayloadSize(void)
{
  ...
  csn(LOW);
  doTransfer( R_RX_PL_WID );
  result = doTransfer(0xff);
  csn(HIGH);
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  doTransfer( W_ACK_PAYLOAD | ( pipe & B111 ) );
  const uint8_t max_payload_size = 32;
